...
export default function DisplayAllOrders() {
  ...
  async function allPr(){
    ...
    try {
        ...
        const endpoint = `/orders/allForUser/${Id}`;
  ...
  */
  ...
  const orderDeliveryTime = new Date(id.orderDate);
  const timeDifferenceInMilliseconds = currentTime - orderDeliveryTime;
  const timeDifferenceInHours = timeDifferenceInMilliseconds / (1000 * 60 * 60); // pretvaranje u sate
      ...
      <Table size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell align="right" sx={{height: '5px'}}></TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
...
<TableCell align="center">
    ...
    <Dialog
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-slide-description">
          You cannot delete an order after one hour!
          </DialogContentText>
        </DialogContent>
        <DialogActions>
